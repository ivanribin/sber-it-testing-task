...
All simulation events generated by the Mock Aggregator are logged to the browser console.

A deliberately simple logging approach (`console.info`) is used to provide visibility into internal system behavior, including:

- task creation events
- task updates
- task deletion events

The purpose of logging is to make the simulation process observable during development and manual testing.

A more advanced logging solution (structured logger, log levels, external monitoring, etc.) was intentionally not introduced, as the original assignment did not require production-grade logging infrastructure.

This keeps the implementation lightweight while still allowing easy debugging and verification of aggregator activity.
